...
class PwmIn:
	...
	def getDutyAdjusted(self):
        ...
        def getDuty(self):
            ...
            return 100.0*(self.pulsewidth/self.period)
	...
	def edge(self):
		...
		self.tmp=self.reading-self.starttime
		if (self.state==1):
			if (self.tmp>0.019):
				self.risen=True
				self.period=self.tmp
				self.starttime=self.reading
		elif (self.state==0):
			if (self.tmp>0.0005 and self.tmp< 0.0050 and self.risen==True):
				self.pulsewidth=self.tmp
				self.risen=False
	...
	'''def edge(self):
		...
		if (self.started==False):
			...
			if (dif<=self.maxPulse and dif>self.minPulse):
				...
				self.Rh=dif
				if (self.Rh>self.inmax):
					self.inmax=self.Rh
				elif (self.Rh<self.inmin):
					self.inmin=self.Rh
